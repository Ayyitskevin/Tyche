...
```
Key facts a builder must not violate: **`web` never depends on `data-adapters`** (it reaches data only through the API over HTTP/SSE), and **`api` never depends on the browser packages `ui`, `terminal-kernel`, or `module-sdk`**. It DOES depend on `@tyche/analytics` directly (a shared pure-math lib — e.g. `apps/api/src/routes/user.ts` imports `computePortfolioRisk`; `apps/api/package.json` declares it), which is allowed. `contracts` has zero internal dependencies and must stay that way — it is the shared language.
...
- **Provider plane.** 28 typed capabilities (`packages/contracts/src/provider.ts` → `PROVIDER_CAPABILITY_KEYS`). Deterministic `MockProvider` declares **26 of the 28** — everything except `bonds` and `portfolio`, neither of which any command requires (`MOCK_CAPABILITIES`, `packages/data-adapters/src/MockProvider.ts`) — 8 seed instruments + synthesis for any symbol, market sessions, corporate-events calendar. Capability-gap routing (`providerRegistry.ts` → `forCapability` / `servesSymbol` venue scoping), cache interface, reusable `conformance.ts` suite.
...
- **65 stable commands** (recount: `grep -c '^  cmd(' packages/terminal-kernel/src/commands.ts`) with real modules (`apps/web/src/modules/`), each provider-capability-gated with graceful empty states: charting `GP`/`GIP` (candles, axes, volume, SMA/EMA/RSI, crosshair, wheel-zoom/pan, log scale, `vs` comparison overlay), `DES`, `HP`, `QM`, `FOCUS`, `W` (tabs + batch import), `N`/`TOP` (filters), `CF`, `FTS` (filing full-text search), `FA`, `EM`, `ANR`, `HDS`, `OMON`, `TAS`, `BOOK`, `FUND`, `DEX`, `COMM`, `FX`, `HEAT`, `MEMB`, `WEI`, `EVT`, `COMP`, `EQS`, `MOST`, `ECO`, `OVME`, `CALC`, `NOTE`, `PORT` (read-only, no orders), `ALERT`, `AI`, `LAYOUT`, `LAUNCH` (research launchpad), `SETTINGS`, `SECF`, `HELP`, plus hosted `ACCOUNT`/`ADMIN`; and the analytics/forensic/market-structure families `SCORE`, `PERF`, `BETA`, `SEAS`, `MULT`, `DCF`, `WACC`, `RV`, `CORR`, `CARRY`, `LIQ`, `DEXA`, `FLOW`, `INSD`, `INST`, `MEVT`, `EFTS`, `YCRV`, `ECOC`. The exhaustive list drifts as slices land — recount from `DEFAULT_COMMANDS` rather than trust it.
...
- [ ] Never bundle or resell market data. Real feeds are BYO-key or keyless-public adapters; the deterministic MockProvider must keep the whole app usable with zero keys (it declares 26 of 28 capabilities in MOCK_CAPABILITIES — every capability any command requires). *(State of the Project, Gaps & a Sequenced Build Backlog)*
