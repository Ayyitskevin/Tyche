...
- **Billing has monthly + annual prices, single-seat.** `apps/api/src/saas/billing.ts` supports an optional `STRIPE_PRICE_ID_ANNUAL` (annual plan selectable at checkout). Still no team/seat model, no per-seat `ADMIN` count (backlog #8).
