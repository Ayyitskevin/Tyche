...
| `packages/analytics` | `@tyche/analytics` | Pure financial math: indicators, returns, risk, options pricing, portfolio, TVM, screen evaluation. No I/O. | `contracts` | — |
...
1. **Argument-level autocomplete in the command bar** — S/M. *Why:* every data command with a non-symbol arg (ECO series, EQS fields, LAYOUT/watchlist names) benefits; unblocks discoverability. *Reuse:* extend `Suggestion.kind` in `apps/web/src/terminal/suggest.ts` (+ its `.test.ts`), wire in `CommandBarContainer.tsx`; source id lists from the kernel/registry. *Accept:* typing `ECO ` suggests known FRED series; `EQS` suggests filter fields; suggestions rank command→symbol→arg; unit tests cover each; no regression in existing command/symbol tests.

2. **CSV/JSON export parity across every table module** — M. *Why:* provenance-stamped export is a differentiator; several table modules still lack it. *Reuse:* `apps/web/src/modules/export.ts` (already embeds provenance) + `export.test.ts`; the `DataTable` (`packages/ui/src/DataTable.tsx`) call sites. *Accept:* `HP`, `EM`, `ANR`, `HDS`, `OMON`, `MEMB`, `WEI`, `COMM`, `MOST`, `EQS`, `DEX`, `FUND` each expose CSV+JSON export; every export includes the provenance line; one Playwright case asserts a download for a representative table.
...
6. **Trial-ending + welcome-back transactional emails** — M. *Why:* directly targets the LAUNCH Week-3 "first 5 paying" and Week-2 "day-2 return" goals. *Reuse:* `email.ts` sender, trial state in `billing.ts`, a scheduled tick (extend graceful lifecycle in `apps/api/src/index.ts` or a small interval). *Accept:* day-11 "trial ending" mail sent once per account; idempotent (no double-send on restart); audited; disabled when `TYCHE_EMAIL_SINK=console` in a way that warns, not crashes.

7. **Annual Stripe price (second plan)** — S/M. *Why:* "2 months free" annual is the standard conversion lever (LAUNCH Week 3). *Reuse:* `StripeBillingDriver` in `billing.ts` (currently one `priceId`); `ACCOUNT` module + `docs/BILLING.md`. *Accept:* a second env price (`STRIPE_PRICE_ID_ANNUAL`) selectable at checkout; `ACCOUNT` shows both options and current interval; webhook maps either price to `pro`; `billing.test.ts` covers both.
...
11. **SEC company-facts → real `fundamentals` adapter** — L. *Why:* the roadmap's most-requested next real source; makes `FA`/`EM` real for US issuers under a free keyless source. *Reuse:* `SecEdgarProvider.ts` (its EDGAR HTTP client, UA politeness throttle, ticker→CIK map, `FetchLike` injection) — add `fundamentals: true` to its descriptor and implement the method; register via `providerRegistry.ts`; run `conformance.ts`. *Accept:* with `SEC_EDGAR_USER_AGENT` set, `AAPL FA` returns real income/balance/cash-flow with `mode: 'public'` provenance; mock fallback intact when unset; conformance + adapter unit tests pass; no data bundled (fetched live per operator UA).
...
16. **External-SIEM audit sink** — S. *Why:* self-hosters want events off-box; the interface already supports it. *Reuse:* `AuditSink` interface in `apps/api/src/security/audit.ts` (console + file impls exist; call sites unchanged). *Accept:* an HTTP/webhook sink selectable via `TYCHE_AUDIT_SINK`; never throws into the request path; documented in `SECURITY.md`.

17. **Layout keyboard chords (mod+1..9) + workspace-scoped defaults** — S/M. *Why:* keyboard-first is the core promise; ROADMAP Week-4 item. *Reuse:* shortcuts model `packages/terminal-kernel/src/shortcuts.ts`, `LayoutManagerModule.tsx`, SETTINGS rebinding. *Accept:* mod+1..9 switch named layouts; rebindable in `SETTINGS`; persisted in preferences; unit-tested.
...
- Is real fundamentals expected strictly from SEC company-facts (US issuers only), or is global coverage in scope? SEC company-facts covers US filers only, which leaves non-US names mock-only.
