...
export const FORMULAS: Record<string, FormulaDefinition> = {
  ...
  'flow.trade-tape.v1': {
    ...
    units: 'dimensionless',
    assumptions: [
      'Drops non-positive price/size prints before aggregation',
      'VWAP = Σ(price×size) / Σ size; null when totalVolume = 0',
      'buyShare = buyVolume / (buyVolume + sellVolume) over classified sides only',
      'unknown aggressor side is tallied separately and never imputed as buy or sell',
      'netNotional = Σ sign × price × size with buy +, sell −, unknown 0',
    ],
    limitations: [
      'Snapshot aggregate only — no clock, no order-book reconstruction',
      'Venue side classification quality is not validated here',
    ],
    authority: 'packages/analytics/src/tradeFlow.ts (descriptive tape aggregates as coded)',
    needsHumanReview: false,
    disclaimer: DISCLAIMER,
  },
  'dex.pool-structure.v1': {
    id: 'dex.pool-structure.v1',
    name: 'DEX pool structure (LWAP, HHI, turnover)',
    module: 'dexAnalytics.ts',
    units: 'currency',
    assumptions: [
      'LWAP over pools with both price and positive liquidity',
      'Missing price/liquidity/volume skipped — never treated as zero depth',
      'HHI = Σ (liqShare)² over positive-liquidity pools',
      'buyShare from reported buys24h/sells24h only when both present',
    ],
    limitations: [
      'Cross-venue USD figures depend on upstream Dexscreener normalization',
      'No MEV / fee-tier adjustment',
    ],
    authority: 'packages/analytics/src/dexAnalytics.ts',
    needsHumanReview: false,
    disclaimer: DISCLAIMER,
  },
  'risk.sharpe.v1': {
    ...
    units: 'ratio',
    assumptions: [
      'excess = return − rf/periodsPerYear; Sharpe = mean(excess)/stddev(excess)×√periods',
      'Null when n<2 or excess stddev is 0',
    ],
    limitations: ['Sample stddev; no HAC / Newey–West adjustment'],
    authority: 'packages/analytics/src/risk.ts',
    needsHumanReview: false,
    disclaimer: DISCLAIMER,
  },
  'risk.series-stats.v1': {
    id: 'risk.series-stats.v1',
    name: 'Headline series stats (return/vol/DD/Sharpe)',
    module: 'risk.ts',
    units: 'ratio',
    assumptions: ['Simple total return from first to last close; vol and Sharpe on simple returns'],
    limitations: ['Does not annualize total return'],
    authority: 'packages/analytics/src/risk.ts',
    needsHumanReview: false,
    disclaimer: DISCLAIMER,
  },
  'risk.performance.v1': {
    id: 'risk.performance.v1',
    name: 'Multi-horizon performance snapshot',
    module: 'performance.ts',
    units: 'ratio',
    assumptions: [
      'Trailing returns anchored to last candle date (not wall clock)',
      'YTD uses last close of prior calendar year',
      'Horizon null when history does not reach the cutoff',
    ],
    limitations: ['Assumes daily bars for vol/Sharpe annualization (252)'],
    authority: 'packages/analytics/src/performance.ts',
    needsHumanReview: false,
    disclaimer: DISCLAIMER,
  },
};
