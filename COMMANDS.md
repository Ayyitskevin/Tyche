...
| `CARRY` | `FUNDINGANALYTICS`, `FUNDX`, `BASIS` | funding-analytics | | `fundingRates`         | Perp funding carry analytics — daily/annualized carry, mark-vs-index premium, regime label, cross-sectional richness (median deviation + percentile). Descriptive, not advice |
...
| `EQS`   | `SCREEN`, `SCREENER` | screener      |      | `screener`                   | Equity screener — filter/sort the universe by price/%chg/mktcap/volume/sector plus forensic **Altman Z′** (distress), **Piotroski F** (quality) and **Beneish M** (manipulation risk) scores derived from filings, with one-click Distress/Quality/Manip.-risk presets |
